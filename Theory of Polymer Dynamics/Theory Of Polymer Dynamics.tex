...
Construction of the Rouse ring is done by setting the first bead at random position $x_0$, adding a phantom bead $N+1$ and setting its position to $x_0$. Then, we connect bead 1 and the phantom bead by a random walk,$ w(t)$, of $N$ steps using the Brownian bridge. 

The \textbf{Brownian bridge} which starts at $x$ at time 0 and ends in point $y$ at time $t_0$ is given by the general form 
\begin{equation*}
x(t)= \sqrt{2D}w(t)-(t/t_0)[\sqrt{2D}w(t_0)-y+x]+x \qquad 0<t\leq t_0
\end{equation*} 
For the Rouse ring we set $x=y$ and calculate the probability function 
\begin{equation*}
Pr(x(t)<X)=Pr(w(t)<(X-x)/\sqrt{2D}+(t/t_0)w(t_0))
\end{equation*}
so, the probability is given by the probability that the random walk,$w(t)$, is lower than the line $(X-x)/\sqrt{2D}+(t/t_0)w(t_0)$. This can be calculated using the cylindrical sets $C(t,[a,b])$, which has a Weiner measure 
\begin{equation*}
Pr(C(t,[a,b]))=\frac{1}{\sqrt{2\pi t}}\int_{a}^{b}e^{-x^2/2t}dx
\end{equation*}
setting $a=0, b=(X-x)/\sqrt{2D}+(t/t_0)w(t_0)=\alpha+\beta t$, results in 
\begin{equation*}
F=Pr(C(t,[0,\alpha+\beta t]))=\frac{1}{\sqrt{2\pi t}}\int_{0}^{\alpha+\beta t}e^{-x^2/2t}dx
\end{equation*}
The probability distribution is then 
\begin{equation*}
\frac{\partial F}{\partial x}=f=\frac{1}{\sqrt{2\pi t}}[e^{-(\alpha +\beta t)^2/2t}]
\end{equation*}
in three-dimensional space we have
\begin{equation*}
f(t)=\left(\frac{1}{\sqrt{2\pi t}}\right)^3[e^{-3(\alpha +\beta t)^2/2t}]
\end{equation*}

Now that we have the probability density in equilibrium of the Brownian bridge we can calculate average quantities related to the positions of the beads. 

The mean square distance between bead 1 and $1<j\leq N$ is given by the relation 
\begin{equation*}
\left<r\right>^2 = \int r^2f(r)dx
\end{equation*}
[UNFINISHED]
