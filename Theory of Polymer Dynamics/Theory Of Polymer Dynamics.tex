...
Te inverse, $C_\Delta ^{-1}$ is of the form [Show!]
\begin{equation*}
C_\Delta^{-1}(s,t)=
\begin{cases}
\alpha & s=t\\
...
\begin{equation*}
C_\Delta^{-1}(s,t)=(1/N)(
\left(\frac{N}{1-N}\right)\omega_1^{s+t-2}+\sum_{n=2}^N\omega_n^{s+t-2}) 
\end{equation*}
$s=1,..,N, t=1,..,N$
Therefore, the configuration distribution in 3D can be written as 
\begin{equation*}
f_\Delta(\vec{x})=\left(\frac{N}{(2\pi)^N (N-1)} \right)^{1.5}\exp(-0.5\vec{x}\cdot\vec{\delta S(\alpha)})
\end{equation*}
where
\begin{equation*}
\vec{\delta S(\alpha)}=\left[
\begin{matrix}
...
\right]
\end{equation*}
Note that when $\alpha=1$ the system becomes decoupled. 


It can be shown, by a series of elementary operations on $C_\Delta$ that the diagonalized form of $C_\Delta$ is 
\begin{equation*}
C_\Delta(s,t) =
-1, \quad s = t, s=t-1, s=1..N\\
\end{equation*}
for an even $N$, and 
\begin{equation*}
C_\Delta(s,t)=
\begin{cases}
\sigma_\Delta^2 & s=t=1\\
-1              & s=t,s=t-1,s\geq 2
\end{cases}
\end{equation*}
for $N$ odd. 
The eigenvalues $\lambda_j=-1\quad j=1..N$ for $N$ even, and $\lambda_1=\sigma_\Delta^2, \lambda_j=-1 \quad j=2,..N$ for $N$ odd. 
Therefore 
\begin{equation*}
|C_\Delta| =\begin{cases}
1 & $N$\quad odd\\
\sigma_\Delta^2 & $N$ \quad even
\end{cases}
\end{equation*}

The inverse of $C_\Delta$ can be computed to be 
\begin{equation*}
C^{-1}_\Delta=
\begin{cases}
(-1)^{s+t-1} & s\leq t\\
...
Therefore, the configuration distribution (the joint pdf) of the Rouse ring is given by 
\begin{equation*}
f_\Delta(\vec{x})=\frac{1}{\sqrt{(2\pi)^N}}\exp(-0.5x^T C_\Delta^{-1}x)
\end{equation*}
