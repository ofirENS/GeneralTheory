...
Construction of the Rouse ring is done by setting the first and last beads at position $x_0=0$. This forms a discrete \textbf{Brownian bridge}, which is defined using the random walk, $w(t)$, of $N$ steps (between $N+1$ beads). 
\begin{equation*}
...
C_{B}  & = & E((B(t)-E[B(t)])(B(s)-E[B(s)])\\
   ...
   & = & min(s,t)-2st/N+st/N^2=min(s,t)-(st/N^2)(2N-1)
\end{eqnarray*}

We therefore see that the position of the Rouse ring are dependent. The variance is given by setting $s=t$ to be $\sigma^2_{B_t}=t-(t/N)^2(2N-1)$.

The Rouse ring is a Gaussian process with a joint pdf given by  
\begin{equation*}
f_B(x)=\frac{1}{\sqrt{((2\pi)^N|C|)}}\exp(-0.5x^TC^{-1}x)
\end{equation*}
[UNFINISHED]
...
\begin{equation*}
\sigma^2_{\Delta}=1-(1/N^2)(2N-1)
\end{equation*}

The joint pdf of the increments
\begin{equation*}
f_\Delta(x) = \left(\frac{1}{\sqrt{((2\pi)^N |C_\Delta)|}}\right)\exp(-0.5x^TC_\Delta^{-1}x)
\end{equation*}

To find the pdf we need to estimate $|C_\Delta|$ and $C^{-1}$, the form of $C_\Delta$ is 
\begin{equation*}
C_\Delta = \left[
\begin{matrix}
\sigma^2_{\Delta} & k(N)                & k(N)                 & ... & k(N) \\
k(N)                  &\sigma^2_{\Delta}& k(N)                 & ... & k(N) \\
k(N)                  & k(N)                &\sigma^2_{\Delta} & ... & k(N) \\
.                     &    .                &                  &     &      \\
.                     &                     &  .               &     &      \\
.                     &                     & k(N)             &\sigma^2_{\Delta} & k(N)  \\
k(N)                  & ...                 &...               & k(N)&\sigma^2_{\Delta}
\end{matrix}
\right]
\end{equation*}
with $k(N)=-(1/N^2)(2N-1) $. The inverse of $C_\Delta$ will be computed using the eigenvalues and eigenvectors of a circulant matrix. 

The normalized eigenvectors of a circulant matrix are given by 
\begin{equation*}
v_j = \frac{1}{\sqrt{N}}(1,\omega_j,\omega^2_j,...,\omega^{N-1}_j) \quad , j=0,1,..,N-1
\end{equation*}
with $\omega_j = \exp(\frac{2\pi ij}{N})\quad j=0,..,N-1$,  $i=\sqrt{-1}$. The corresponding eigenvalues are
\begin{equation*}
\lambda_j = c_0+c_{N-1}\omega_j+c_{N-2}\omega^2_j+...+c_1\omega^{N-1}_j \quad j=0,..,N-1
\end{equation*}

with $c_0, c_{N-1}, c_{N-2},...,c_1$, the first row of $C_\Delta$
therefore, the determinant is 
\begin{equation*}
|C_\Delta| = \prod_{j=0}^{N-1}\lambda_j
\end{equation*}

In our case $c_0=\sigma^2_\Delta$, $c_{N-1}=...=c_1=k(N)$, therefore 
\begin{eqnarray*}
\lambda_j & = & \sigma^2_\Delta+k(n)\sum_{n=1}^{N-1}\omega_j^n
           =  \sigma^2_{\Delta} +k(n)\sum_{n=1}^{N-1}\exp(\frac{2\pi nij}{N})\\
          & = & \sigma^2_{\Delta} +k(n)\sum_{n=1}^{N-1}\exp(\frac{2\pi nij}{N})\\
          & = & \sigma^2_{\Delta} +k(n)\frac{\exp(\frac{2\pi ij}{N-1})-\exp(\frac{2\pi Nij}{N-1})}{1-\exp(\frac{2\pi ij}{N-1})}\\
          & = & k(N)\frac{1-\exp(\frac{2\pi Nij}{N-1})}{1-\exp(\frac{2\pi ij}{N-1})}+1
\end{eqnarray*}

We give now the list of eigenvalues 
\begin{eqnarray*}
\lambda_0 & = & \sigma^2_\Delta-(1/N^2)(2N-1)(N-1)\\
\lambda_1 & = & \sigma^2_\Delta-(1/N^2)(2N-1)
\end{eqnarray*}
